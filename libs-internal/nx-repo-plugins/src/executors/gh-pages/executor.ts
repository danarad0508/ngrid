...
import {
  CopyFile,
  CopyPattern,
} from 'ng-cli-packagr-tasks/dist/tasks/copy-file';

import { GhPagesExecutorSchema } from './schema';

export default async function executor(
  options: GhPagesExecutorSchema,
  ...
  const steps: {
    title: string;
    disabled?: boolean;
    factory: () => Promise<AsyncIterableIterator<{ success: boolean }>>;
  }[] = [
    {
    ...
    },
    {
      ...
      factory: () =>
        runExecutor(
          parseTargetString(options.serverTarget),
        ...
        runExecutor(
          {
            ...
            target: createSsrCompilerTarget(options, context),
          },
          {},
          context
        ),
    ...
    },
    {
      ...
      factory: () =>
        runExecutor(
          {
            project: context.projectName,
            target: createLocalGhServerTarget(
              options,
              context,
              buildProject.outputPath
            ),
          },
          {},
          context
...
async function executeCopyAssets(
  options: GhPagesExecutorSchema,
  context: ExecutorContext
) {
  return (async function* () {
    ...
    try {
      await CopyFile.executeCopyPatterns(
        copyPatterns,
        context.root,
        copyOptions,
        onCopy
...
function createSsrCompilerTarget(
  options: GhPagesExecutorSchema,
  context: ExecutorContext
) {
...
function createLocalGhServerTarget(
  options: GhPagesExecutorSchema,
  context: ExecutorContext,
  outputPath: string
) {
